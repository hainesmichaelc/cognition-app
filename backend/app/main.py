...
async def connect_repo(request: ConnectRepoRequest):
    try:
        ...
        async with httpx.AsyncClient() as client:
            headers = {"Authorization": f"token {request.githubPat}"}
            
            repo_response = await client.get(f"https://api.github.com/repos/{owner}/{name}", headers=headers)
            ...
            repos_store[repo_id] = {
                ...
                "openIssuesCount": len(issues_data)
            }
            
            processed_issues = []
            for issue in issues_data:
                if "pull_request" not in issue:  # Skip PRs
                    age_days = (datetime.now() - datetime.fromisoformat(issue["created_at"].replace("Z", "+00:00"))).days
...
async def resync_repo(repo_id: str, request: ResyncRequest):
    ...
    return {"message": "Repository resynced successfully"}
...
async def send_message_to_devin(session_id: str, request: MessageRequest):
    ...
    if session["structured_output"]:
        session["structured_output"]["progress_pct"] = min(session["structured_output"]["progress_pct"] + 20, 90)
